...
open(FIN, "features/addDiscourse/resources/connectives.txt") or die "Couldn't open connectives.txt";
foreach my $c (<FIN>) {
  chomp($c);
...
open(FIN, "features/addDiscourse/resources/connectiveTexts.txt") or die "Couldn't open connectiveTexts.txt";
foreach my $c (<FIN>) {
  chomp($c);
  $connectiveTexts{$c} = 1;
}
...
open(FIN, "features/addDiscourse/resources/connectives.info") or die "Couldn't open classifier.info";
my $currLabel = "";
foreach my $line (<FIN>) {
  chomp($line);
