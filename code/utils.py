...
from score_ranges import score_ranges, wc_score_ranges
...
def padding_sentence_sequences_without_mask(index_sequences, max_sentnum, max_sentlen, dim=1, dtype=np.int32):

    X = np.empty([len(index_sequences), max_sentnum, max_sentlen, dim], dtype=dtype)
...
def build_embedding_table(word_alphabet, emb_dict, emb_dim, logger, caseless):
    ...
    oov_num = 0
    for index, word in word_alphabet.items():
...
def get_model_friendly_scores(scores_array, prompt_id, use_wc=False, min_score=None, max_score=None):
    if (min_score is not None) and (max_score is not None):
        low = min_score
        high = max_score
    else:
        if use_wc:
            low, high = wc_score_ranges[prompt_id]
        else:
...
def convert_to_dataset_friendly_scores(scaled_scores, prompt_id, use_wc=False):
...
def scale_feature(x_train, x_dev, x_test):
    scaler = preprocessing.MinMaxScaler().fit(x_train)
    train_out = scaler.transform(x_train)
    dev_out = scaler.transform(x_dev)
    test_out = scaler.transform(x_test)

    return train_out, dev_out, test_out



def create_tokenizer_from_transformers(module_handle):
    return AutoTokenizer.from_pretrained(module_handle)


def convert_bert_sequence(index_sequences, scores, max_sentnum, max_sentlen):
    input_ids = np.zeros([len(index_sequences), max_sentnum, max_sentlen], dtype='int32')
    segment_ids = np.zeros([len(index_sequences), max_sentnum, max_sentlen], dtype='int32')
    input_masks = np.ones([len(index_sequences), max_sentnum, max_sentlen], dtype='int32')
    labels = np.empty([len(index_sequences), 1], dtype=np.float32)
    ...
    for i in range(len(index_sequences)):
        ...
        for j in range(num):
            ...
            # Zero out X after the end of the sequence
            input_ids[i, j, length:] = 0
            # Make the mask for this sample 1 within the range of length
            input_masks[i, j, :length] = 1

        input_ids[i, num:, :] = 0
        labels[i] = scores[i]
    return input_ids, segment_ids, input_masks, labels
